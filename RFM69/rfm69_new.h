...
#define RFM69_FIFO_SIZE 64 // max number elements FIFO can hold
#define RFM69_SPI_WRITE_MASK 0x80 // write commands to RFM must have this bit
...
// to initialize the RFM69 using SPI
...
// functions pertaining to the RFM69
...
struct rfm69{
	uint8_t DATA[MAX_MSG_LEN]; // message to be sent/received
	uint8_t _mode; // current mode;
	uint8_t senderID;
	uint8_t targetID;
	uint8_t payloadLen;
	uint16_t RSSI; // RSSI during reception
	uint8_t _powerLevel; // power level (31) in LowPowerLab
};

rfm_status_t rf69_init(){
    ...
    rf69_set_mode(RFM69_MODE_SLEEP); // TODO: determine if this should be still SLEEP or STDBY
...
static rfm_status_t _rf69_read(const uint8_t reg, uint8_t* result){
    ...
    snprintf(buffer, 50, "\rRead reg 0x%02X and recv 0x%02X\n\r", reg, *result);
    serial_outs(buffer); // for debugging
...
static rfm_status_t _rf69_write(const uint8_t reg, const uint8_t val){
    ...
    spi_ss_assert();
    /* Transmit the reg address to the RFM69. */
    spi_exchange_single(reg | RFM69_SPI_WRITE_MASK, &data);
    /* Transmit the value for this address */
    spi_exchange_single(val, &data);
...
// change data from uint8_t to char
...
rfm_status_t rf69_interruptHandler(void){
    uint8_t res;
    _rf69_read(REG_IRQFLAGS2, &res);
    // serial_outs("\rinterrupt handler\n\r");
    //if(currentMode == RFM69_MODE_TX && (res & 0x04)){
    if(_mode == RFM69_MODE_TX && (res & 0x04)){
